...
class CalcParser(Parser):
    ...
    def declaration(self, p):
        ...
        l = len(declarationList)
        for i in declarationList:
            if i[1] == 'var':
                VarDecNode(i[0])
            elif i[1] == 'pointer':
                PointerNode(i[0], 'NULL')
            elif i[1] == 'ini':
                obj = VarDecNode(i[0])
                AssignNode(obj, i[2])
            elif i[1] == 'inipointer':
                obj = PointerNode(i[0], i[2])
                #AssignNode(obj, i[2])
    ...
    def variable(self, p):
        if p.empty1:
            if p.empty4:
                global declarationList
                declarationList.append((p.ID, 'var'))
            else:
    ...
    def variable(self, p):
        if p.empty2:
            if p.empty5:
                global declarationList
                declarationList.append((p.ID, 'ini', p.expr))
            else:
                print("error: conflicting types for", p.ID)
        else:
            print("Error.", p.ID, "redeclared")
    ...
    def pointer(self, p):
        if p.empty1:
            if p.empty4:
                global declarationList
                declarationList.append((p.ID, 'pointer'))
            else:
                print("Error:", p.ID, "redeclared")
        else:
            print("error: conflicting types for", p.ID)
    ...
    def pointer(self, p):
        if p.empty2:
            if p.empty5:
                global declarationList
                declarationList.append((p.ID, 'inipointer', p.ID1))
            else:
